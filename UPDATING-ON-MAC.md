...
The updater compiles first. If compilation fails, your running app is left alone. If it succeeds, it asks the old app to quit, packages the new app, copies it to your personal `~/Applications/VolumeGuard.app`, verifies its signature, and opens it. It never force-kills the app. Your live alert history stays in Application Support. Finish any filesystem test/check before updating.
...
```sh
open "$HOME/Applications/VolumeGuard.app" --args --demo
```
