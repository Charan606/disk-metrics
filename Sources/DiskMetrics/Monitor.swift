...
final class Monitor: ObservableObject {
    ...
    var visibleVolumes: [Volume] {
        showSystemVolumes ? volumes : volumes.filter { !$0.path.hasPrefix("/System/Volumes/") }
    }
    ...
    var warning: Bool {
        volumes.contains { !$0.path.hasPrefix("/System/Volumes/") && $0.fraction * 100 >= threshold }
    ...
    func refresh() async {
        ...
        for volume in snapshot.volumes {
            ...
            if volume.path.hasPrefix("/System/Volumes/") { continue }
        ...
        for volume in previous?.volumes ?? [] where !current.contains(volume.path) {
            addIncident(volume.path, "Mount is no longer observed. Check whether it was intentionally unmounted or whether the connection failed.")
